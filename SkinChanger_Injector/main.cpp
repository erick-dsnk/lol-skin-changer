...
void auto_update()
{
	...
	try
	{
		...
		if (std::regex_search(json, tagnameMatch, tagnameRegex))
		{
			...
			if (std::regex_search(json, dateMatch, dateRegex))
			{
				...
				auto date_of_current_release = System::IO::File::GetLastWriteTime(L"SkinChanger.dll").ToString(L"dd.MM.yyyy");
				if (date_of_current_release != date_of_new_release)
				{
					auto result = LI_FN(MessageBoxW)(nullptr, L"New version is available on GitHub\nWould you like to download it now?", L"SkinChanger", MB_YESNO | MB_ICONINFORMATION);
					if (result == IDYES)
					{
						if (std::regex_search(json, urlMatch, urlRegex))
						{
							auto url = gcnew String(urlMatch[1].str().c_str());
							client->DownloadFile(url, String::Format(L"SkinChanger_{0}.zip", version));
							Environment::Exit(0);
						}
					}
				}
	...
	{
		MessageBox::Show(e->Message, L"SkinChanger", MessageBoxButtons::OK, MessageBoxIcon::Error);
	}
}
...
int main([[maybe_unused]] array<String^>^ args)
{
	...
	SkinChangerInjector::SkinChangerUI form;

	auto thread{ std::thread(Injector::run) };
	auto screenThread{ gcnew Thread(gcnew ThreadStart(%form, &SkinChangerInjector::SkinChangerUI::updateScreen)) };
	screenThread->Start();
