	...
	if (LI_FN(K32EnumProcessModules)(hProcess, hMods, sizeof(hMods), &cbNeeded)) {
		for (auto i{ 0u }; i < (cbNeeded / sizeof(HMODULE)); ++i) {
			...
			if (LI_FN(K32GetModuleBaseNameW)(hProcess, hMods[i], szModName, sizeof(szModName) / sizeof(TCHAR))) {
				if (std::wcscmp(szModName, L"SkinChanger.dll") == 0) {
...
bool WINAPI Injector::inject(const std::uint32_t pid) noexcept
{
	...
	const auto dll_path{ std::wstring(current_dir) + L"\\SkinChanger.dll" };

	if (auto f{ std::ifstream(dll_path) }; !f.is_open()) {
		LI_FN(MessageBox)(nullptr, L"SkinChanger.dll file could not be found.\nTry reinstalling the cheat.", L"SkinChanger", MB_ICONERROR | MB_OK);
...
// safety mechanism so the executable isn't detected by Riot's anti cheat system
...
void Injector::run() noexcept
{
	...
	while (true) {
		...
		SkinChangerInjector::gameState = (league_processes.size() > 0) ? true : false;
		SkinChangerInjector::clientState = (league_client_processes.size() > 0) ? true : false;
		...
		for (auto& pid : league_processes) {
			if (!Injector::isInjected(pid)) {
				SkinChangerInjector::cheatState = false;
				if (SkinChangerInjector::btnState) {
					std::this_thread::sleep_for(1.5s);
					if (Injector::inject(pid))
						SkinChangerInjector::cheatState = true;
					else
						SkinChangerInjector::cheatState = false;
				}
